...
void Bundle::AddMeas(int nCam, int nPoint, Vector<2> v2Pos, double dSigmaSquared, bool _bLeftCam/*=true*/)
{
...
inline void Bundle::ProjectAndFindSquaredError(Meas &meas)
{
  ...
  if(meas.bLeftCam){
    ...
    se3Cam = mCamera_Left.GetExtrinsic() * cam.se3CfW;
    mCam = &mCamera_Right;
  }
  // Project the point.
  meas.v3Cam = se3Cam * point.v3Pos;
  if(meas.v3Cam[2] <= 0)
    {
      //cerr << "map point from " << (meas.bLeftCam ?  "left " : "right " ) << "bad" << endl;
      meas.bBad = true;
      return;
    }
  meas.bBad = false;
  Vector<2> v2ImPlane = project(meas.v3Cam);
  Vector<2> v2Image   = mCam->Project(v2ImPlane);
  meas.m2CamDerivs = mCam->GetProjectionDerivs();
  meas.v2Epsilon = meas.dSqrtInvNoise * (meas.v2Found - v2Image);
  //if(meas.v2Epsilon[0] > 20 || meas.v2Epsilon[1] > 20 || meas.v2Epsilon[0] < -20 || meas.v2Epsilon[1] < -20)
    //cerr << "map point from " << (meas.bLeftCam ?  "left " : "right " ) << "gave bad epsilon" << endl; 
...
bool Bundle::Do_LM_Step(bool *pbAbortSignal)
{
  ...
  //  Do a LM step according to Hartley and Zisserman Algo A6.4 5 in MVG 2nd Edition
  ...
  for(list<Meas>::iterator itr = mMeasList.begin(); itr!=mMeasList.end(); itr++)
      ...
      else 
	{
      ...
      // differentiate du/dx_i and dv/dx_i where x_i is the ith 
      // coordinate of the world point 
      // matrix ordering = { du/dx, dv/dx; du/dy, dv/dy; du/dz..

      Matrix<3> m6Motion;
      if(meas.bLeftCam)
	m6Motion = cam.se3CfW.get_rotation().get_matrix();
      else
	m6Motion = (mCamera_Left.Extrinsic * cam.se3CfW).get_rotation().get_matrix();
      
      for(int m=0;m<3;m++)
	{
	  const Vector<3> v3Motion = m6Motion.T()[m];
  ...
  for(list<Meas>::iterator itr = mMeasList.begin(); itr!=mMeasList.end(); itr++)
    {
      ...
      ATANCamera *mCam;
      if(!meas.bLeftCam){
	//cerr << "THIS SHOULD NOT HAPPEN!!!!!! " << endl << endl << endl;
	se3Cam = mCamera_Left.Extrinsic * se3Cam;
	mCam = &mCamera_Right;
      }else{
	mCam = &mCamera_Left;
      }
...
void Bundle::GenerateMeasLUTs()
{
  ...
  for(unsigned int nCam = 0; nCam < mvCameras.size(); nCam++)
    {
      // For each camera- make a vector of measurement pointers
  ...
  // for the nth cam vector and the mth map point that the cam
  // observes create a pointer to the measurement
}
