...
public:
  ...
  Bundle(const ATANCamera &TCamL, const ATANCamera &TCamR);   // We need the camera model because we do full distorting projection in the bundle adjuster. Could probably get away with a linear approximation.
  ...
  void AddMeas(int nCam, int nPoint, Vector<2> v2Pos, double dSigmaSquared, bool bLeftCam = true); // Add a measurement
