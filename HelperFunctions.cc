/* These function are helpers for performing some basic features of stereo */
...
// return the skewsymmetric form of a vector

TooN::Matrix<3> SkewSymmetric(TooN::Vector<3> v3)
{
  TooN::Matrix<3> m3SkewSymm = Zeros;
  m3SkewSymm[0] = makeVector(0,-v3[2],v3[1]);
  m3SkewSymm[1] = makeVector(v3[2],0,-v3[0]);
  m3SkewSymm[2] = makeVector(-v3[1],v3[0],0);
  return m3SkewSymm;
}

// function which performs stereo rectification - based on the 
// opencv implementation but including the FOV distortion model
// rather than the zhang model
		       ...
		       CvRect* roi1, CvRect* roi2 )
{
  ...
  for( k = 0; k < 2; k++ )
      ...
      for( i = 0; i < 4; i++ )
	{
      ...
      // undistort the points with the FOV model
  ...
  alpha = MIN(alpha, 1.);
  // undistorted  boundaries of the images
...
// undistort the edge of the grid using teh FOV model
...
// undistort points using the FOV model
...
// generate the rectification map by:
// unproject from real image
// rotate-translate
// project to stereo image
