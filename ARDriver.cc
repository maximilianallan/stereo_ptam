...
ARDriver::ARDriver(const StereoCamera &cam, ImageRef irFrameSize, GLWindow2 &glw)
  :mCamera(cam), mGLWindow(glw)
{
  mirFrameSize = irFrameSize;
  mCamera.Left().SetImageSize(mirFrameSize);
  mbInitialised = false;
}
...
void ARDriver::Render(Image<Rgb<byte> > &imFrame, SE3<> se3CfromW)
{
  ...
  glLoadIdentity();
  glMultMatrix(mCamera.Left().MakeUFBLinearFrustumMatrix(0.005, 100));
  ...
  if(bFirstRun)
	...
	{  
	  ...
	  for(int xstep = 0; xstep <= nStepsX; xstep++)
		...
		Vector<2> v2UFBUnDistorted = mCamera.Left().UFBLinearProject(mCamera.Left().UFBUnProject(v2UFBDistorted));
